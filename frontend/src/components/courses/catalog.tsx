...
import { LangBadge } from "./chrome";
import type { EnrolledCourse, CatalogCourse, Track, ActivityItem, Build } from "./data";
...
export function EnrolledCard({ course, isActive }: EnrolledCardProps) {
  const done = course.completed;
  const isWorkspace = course.id === "http-server";
  const href = isWorkspace ? "/workspace-v2" : "#";
  return (
    <Link
      href={href}
...
export function ActivityStream({ items }: ActivityStreamProps) {
  const kindConfig: Record<ActivityItem["kind"], { dot: string; label: string }> = {
...
export function ActivityHeatmap({ counts }: ActivityHeatmapProps) {
  ...
  const bucket = (c: number | null): number => {
    if (c == null) return -1;
    if (c === 0) return 0;
    const p = c / max;
    if (p < 0.25) return 1;
  ...
  const bucketBg = (b: number): string => {

...
export function WorkspacePreview() {
  return (
    <div className="flex h-full w-full flex-col bg-bg text-text" aria-hidden>
      {/* Top context bar */}
      <div className="flex h-7 flex-shrink-0 items-center justify-between border-b border-border bg-surface px-3 font-mono text-[10px] text-text-dim">
        <div className="flex items-center gap-2 truncate">
          <span className="text-primary/80">◢</span>
          <span className="truncate text-text-muted">Crea tu Servidor HTTP</span>
        ...
        <div className="flex flex-shrink-0 items-center gap-3">
          <div className="flex items-center gap-1.5">
            <span className="tabular-nums text-text-muted">18</span>
            <span className="text-border">/</span>
            <span className="tabular-nums">15</span>
            <span className="ml-1 h-[3px] w-16 bg-border">
              <span className="block h-full w-[80%] bg-primary" />
            </span>
          </div>
          <span className="rounded border border-border px-1.5 py-[1px] text-[9px] tracking-widest">
            ⌘K
          </span>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        {/* Icon rail */}
        <div className="flex w-10 flex-shrink-0 flex-col items-center justify-between border-r border-border bg-surface py-2">
          ...
          <div className="flex flex-col items-center gap-0.5">
            <button className="flex h-9 w-9 flex-col items-center justify-center bg-primary text-[8px] font-medium uppercase tracking-widest text-text">
        ...
        {/* Modules panel */}
        <div className="flex w-[180px] flex-shrink-0 flex-col border-r border-border bg-surface">
          <div className="flex h-7 flex-shrink-0 items-center justify-between border-b border-border px-3 font-mono text-[9px] uppercase tracking-[0.2em] text-text-dim">
            <span>Modules</span>
            <span className="rounded-sm bg-bg px-1 py-[1px] text-[8px] normal-case text-text-muted">
              go
            </span>
          </div>
          <div className="flex flex-col overflow-hidden px-2 py-2 font-mono text-[10px]">
            <div className="mb-2 px-1 text-[10px] font-medium leading-tight text-text">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex h-7 flex-shrink-0 items-center gap-3 border-b border-border bg-surface px-3 font-mono text-[10px]">
            <span className="text-[9px] uppercase tracking-[0.22em] text-text-dim">
              Task
            </span>
            <span className="truncate text-text">
              Protección contra path traversal
          ...
          {/* Tab bar */}
          <div className="flex h-7 flex-shrink-0 items-center border-b border-border bg-surface">
          ...
          {/* Editor surface */}
          <div className="relative flex min-h-0 flex-1 overflow-hidden bg-bg font-mono text-[10.5px] leading-[1.55]">
            {/* Line gutter */}
            <div className="flex w-10 flex-shrink-0 flex-col items-end pr-2 pt-2 text-text-dim/70 tabular-nums">
          ...
          {/* Test output panel */}
          <div className="flex h-[110px] flex-shrink-0 flex-col border-t border-border bg-surface">
            <div className="flex h-6 flex-shrink-0 items-center justify-between border-b border-border px-3 font-mono text-[9px] uppercase tracking-[0.22em] text-text-dim">
              ...
              <div className="flex items-center gap-3">
                ...
                <X size={10} className="text-text-dim" strokeWidth={1.5} />
              </div>
            </div>
            <div className="flex-1 overflow-hidden px-3 pt-2 font-mono text-[10px] leading-[1.6]">
              <div className="flex items-center gap-2 text-text-dim">
                <Circle size={8} strokeWidth={1.5} className="text-text-dim" />
                <span>Compilando…</span>
              </div>
              <div className="mt-1 flex items-center gap-2 text-error">
  ...
  return (
    <div
      className={`flex h-9 w-9 flex-col items-center justify-center gap-0.5 text-[8px] uppercase tracking-widest ${
        active ? "bg-bg text-text" : "text-text-dim"
      }`}
    >
      {icon}
      <span>{label}</span>
    ...
    <div className="mb-[1px]">
      ...
      >
        <div className="flex min-w-0 items-center gap-1">
          {expanded ? (
            <ChevronDown size={10} strokeWidth={1.5} className="flex-shrink-0 text-text-dim" />
          ) : (
            <ChevronRight size={10} strokeWidth={1.5} className="flex-shrink-0 text-text-dim" />
          )}
          <span className="truncate text-[10px]">{label}</span>
        </div>
        <span className="flex-shrink-0 text-[9px] tabular-nums text-text-dim">
          {progress}
        </span>
      </div>
      {expanded && subs && (
        <div className="mt-[1px] flex flex-col gap-[1px] pl-4">
          {subs.map((s) => (
            ...
            >
              {s.state === "done" && <span className="text-success">✓</span>}
              {s.state === "active" && <span className="text-primary">⊘</span>}
              {s.state === "locked" && <span className="text-text-dim">○</span>}
              <span className="truncate">{s.label}</span>
              {s.state === "active" && (
                <span className="ml-auto text-[8px] tabular-nums text-primary">1</span>
              )}
            </div>
          ))}
        </div>
...
function TabItem({
  ...
  return (
    <div
      className={`relative flex h-7 items-center gap-1.5 border-r border-border px-2.5 font-mono text-[10px] ${
        active ? "bg-bg text-text" : "text-text-dim"
      }`}
    >
      {active && (
      ...
      {resource ? (
        <BookOpen size={9} strokeWidth={1.5} className="flex-shrink-0 text-text-dim" />
      ) : (
        <span
      ...
      <X
        size={9}
        strokeWidth={1.5}
        className={`ml-0.5 flex-shrink-0 ${active ? "text-text-dim" : "text-transparent"}`}
      />
    </div>
  );
}

...
export interface ByteDiffProps {
  expected: string;
  actual: string;
}

export function ByteDiff({ expected, actual }: ByteDiffProps) {
  const expBytes = useMemo(() => stringToBytes(expected), [expected]);
  const actBytes = useMemo(() => stringToBytes(actual), [actual]);
...
interface ByteRowProps {
  ...
  setHoverByte: (n: number | null) => void;
}

function ByteRow({ start, expBytes, actBytes, diffs, hoverByte, setHoverByte }: ByteRowProps) {
  const ROW = 16;
  return (
    <div style={{
      display: "grid",
...
export function PacketFlow({ active }: PacketFlowProps) {
...
interface NodeProps {
  ...
  icon: "code" | "buffer" | "socket" | "client";
}

function Node({ label, sub, active, done, icon }: NodeProps) {
...
function StreamLine({ show, color, label, desc }: { show: boolean; color: string; label: string; desc: string }) {
  if (!show) return (
...
export interface ConceptGraphProps {
  open: boolean;
  onClose: () => void;
  onJump: (moduleFullId: string) => void;
}

export function ConceptGraph({ open, onClose, onJump }: ConceptGraphProps) {
  ...
  const toXY = (c: Concept) => ({ x: c.x * dims.w, y: c.y * dims.h });

...
export interface RubberDuckProps {
  test?: Test;
  code: string;
  open: boolean;
  onClose: () => void;
...
export function RubberDuck({ test, code, open, onClose }: RubberDuckProps) {
...
Explain in 2-3 short sentences (max 70 words) WHY this test is failing and point at the specific line or function to fix. Be direct and warm. No preamble, no "great question". Start with the root cause.`;
    ...
    (async () => {
      try {
        const complete = typeof window !== "undefined" ? window.claude?.complete : undefined;
        if (!complete) throw new Error("no claude");
...
export function FocusModeOverlay({ active, onExit }: FocusModeOverlayProps) {
...
export function StreakBadge({ passed }: StreakBadgeProps) {

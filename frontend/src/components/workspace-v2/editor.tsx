...
import type { Submodule, Tab, Resource } from "./types";
...
export function TaskBrief({ sub, onOpenFile }: TaskBriefProps) {
...
export function TabBar({ openTabs, activeFile, onActivate, onClose, saving, saved }: TabBarProps) {
  ...
  return (
    <div style={{
      ...
      {openTabs.map(tab => {
        ...
        const name = tab.type === "resource" ? (tab.label || tab.id) : (tab.id.split("/").pop() || tab.id);
        return (
          <div
            key={tab.id}
            ...
            onMouseEnter={(e) => { if (!isActive) (e.currentTarget as HTMLDivElement).style.color = "var(--text-muted)"; }}
            onMouseLeave={(e) => { if (!isActive) (e.currentTarget as HTMLDivElement).style.color = "var(--text-dim)"; }}
          >
            {isActive && (
              <div style={{
...
function tokenize(line: string): Token[] {
  ...
  const tokens: Token[] = [];
  let i = 0;
  const push = (t: string, cls: Token["cls"]) => tokens.push({ t, cls });
  while (i < line.length) {
    const c = line[i];
...
export function Editor({ content, activeLine, highlightLines = [] }: EditorProps) {
...
export function ResourceView({ resource }: ResourceViewProps) {
  const BODIES: Record<string, ResourceBodyMeta> = {
    r1: {

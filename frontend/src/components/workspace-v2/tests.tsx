...
import { RESPONSE_GO } from "./data";
import { PacketFlow } from "./features";
import { RubberDuck } from "./duck";
import type { Submodule, Test, TestState, TestResult } from "./types";
...
export interface TestPanelProps {
  ...
  onContinue: () => void;
  height: number;
  setHeight: (updater: number | ((h: number) => number)) => void;
}

export function TestPanel({ sub, state, onRun, onClose, onContinue, height, setHeight }: TestPanelProps) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      setHeight((h: number) => Math.max(160, Math.min(500, window.innerHeight - e.clientY - 30)));
    };
...
function TestRow({ test, result, index }: TestRowProps) {
  ...
  return (
    <div style={{
      ...
      <div
        ...
        onMouseEnter={(e) => { if (hasDetail) (e.currentTarget as HTMLDivElement).style.background = "var(--surface-hover)"; }}

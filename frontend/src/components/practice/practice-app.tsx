...
interface DailyDrill {
  ...
  avg_time: string;
  your_best: string | null;
  skills: string[];
  prompt_lines: PromptLine[];
}

const DAILY_DRILL: DailyDrill = {
  id: "daily-2026-04-24",
...
function PLangBadge({ lang, lang_label, size = "sm" }: { lang: string; lang_label: string; size?: "sm" | "md" }) {
...
/* ———— TopNav (ported from courses/chrome.jsx) ———— */
function TopNav({ query, setQuery, active = "practice" }: { query: string; setQuery: (v: string) => void; active?: string }) {
  const tabs = [
...
/* ———— Daily drill hero ———— */
function DailyDrillHero({ drill }: { drill: DailyDrill }) {
...
/* ———— Category sidebar ———— */
function CategorySidebar({ active, setActive }: { active: string; setActive: (id: string) => void }) {
  return (
...
function DrillCard({ drill }: { drill: Drill }) {
  const diffColor: string = ({
    easy: "var(--success)",
    medium: "var(--warning)",
    hard: "var(--error)",
  } as Record<string, string>)[drill.difficulty];
  return (
    <div style={{
      padding: "16px 18px",
...
export function PracticeApp() {
  const [tw] = useState(DEFAULT_TWEAKS);
        ...
        <section style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              {/* Difficulty filter (read-only — tweaks panel removed) */}
              <div style={{ display: "flex", gap: 2 }}>
                {(["all", "easy", "medium", "hard"] as const).map(d => (
                  <button key={d} onClick={() => setDifficulty(d)} style={{
                    padding: "5px 10px",
                    background: difficulty === d ? "var(--surface-active)" : "transparent",
                    border: `1px solid ${difficulty === d ? "var(--border-bright)" : "var(--border)"}`,
                    color: difficulty === d ? "var(--text)" : "var(--text-dim)",

/* Sidebar panel: ModuleList / FileTree / ResourceList */
const { useState: useSideState } = React;

function SidebarShell({ view, ...rest }) {
  if (!view) return null;
  return (
    <div style={{
      width: 264,
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {view === "modules" && <ModuleList {...rest} />}
        {view === "files" && <FileTree {...rest} />}
        {view === "resources" && <ResourceList {...rest} />}
      </div>
    </div>
  );
}

function ModuleList({ course, activeSub, setActiveSub }) {
  const [expanded, setExpanded] = useSideState(new Set([course.modules.find(m => m.submodules.some(s => s.active))?.id]));
...
function FileTree({ files, openFile, activeFile, activeSub }) {
  ...
  function Node({ name, node, depth, parentPath }) {
    const [open, setOpen] = useSideState(true);
    if (node.__file) {
      const f = node.__file;
    ...
    return (
      <div>
        ...
        {open && entries.map(([k, v]) => (
          <Node key={k} name={k} node={v} depth={depth + (name ? 1 : 0)} parentPath={(parentPath ? parentPath + "/" : "") + (name || "")} />
        ))}
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "6px 0 16px" }}>
      {entries.map(([k, v]) => <Node key={k} name={k} node={v} depth={0} parentPath="" />)}
      <style>{`
        .hover-row:hover { background: var(--surface-hover) !important; }
      `}</style>
    </div>
...
function ResourceList({ resources, openResource, runHistory }) {
...
window.SidebarShell = SidebarShell;

/* Success overlay + Command palette */
const { useState: uOvState, useEffect: uOvEff, useRef: uOvRef } = React;

function SuccessOverlay({ sub, onContinue, onDismiss }) {
  return (
    <div style={{
...
function CommandPalette({ open, onClose, onJump }) {
  const [query, setQuery] = uOvState("");
  const inputRef = uOvRef(null);
  uOvEff(() => {
    if (open) {
      setQuery("");
  ...
  ].filter(x => !query || x.label.toLowerCase().includes(query.toLowerCase()));
    ...
    >
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "6px 0" }}>
          ...
          {items.map((it, i) => {
            const kindColor = { task: "var(--primary)", file: "var(--info)", action: "var(--warning)", resource: "var(--success)" }[it.kind];
            const KindIcon = { task: Icon.Dot, file: Icon.File, action: Icon.Zap, resource: Icon.Book }[it.kind];
            return (
              <button key={i}

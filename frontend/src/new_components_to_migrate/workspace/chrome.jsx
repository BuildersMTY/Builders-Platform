/* Top-level chrome: ContextBar + IconRail */
const { useState, useEffect, useRef } = React;

function ContextBar({ course, activeModule, activeSub, passedCount, totalCount, onOpenPalette, streak }) {
...
function RailButton({ icon, active, accent, pulse, label, caption, shortcut, onClick }) {
...
function IconRail({ panelView, setPanelView, onRun, neverRun, onOpenGraph }) {
  const toggle = (v) => setPanelView(panelView === v ? null : v);
  return (
    <div style={{
      width: 54,
      flexShrink: 0,
      ...
      <RailButton
        ...
        onClick={onRun}
      />
      <RailButton icon={<Icon.Home size={15} />} label="Exit to courses" caption="exit" onClick={() => { window.location.href = "Courses.html"; }} />
    </div>
  );
}
